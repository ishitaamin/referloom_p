// referloom_frontend/app/(auth)/SignupAlumniScreen.js
...
export default function SignupAlumniScreen() {
  const router = useRouter();
  const { register } = useAuth();
  
  const [form, setForm] = useState({ fullName: "", email: "", password: "", graduationYear: "", company: "", jobRole: "" });
  const [loading, setLoading] = useState(false);

  const handleRegister = async () => {
    if (!form.fullName || !form.email || !form.password || !form.company) return alert("Please fill all required fields.");
  ...
  // UI Structure is identical to Student, but with Alumni inputs:
  return (
    <ScreenWrapper>
     <KeyboardAvoidingView style={styles.safe} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subHeader}>Join to mentor students and refer talent.</Text>

        <TextInput style={styles.input} placeholder="Full Name *" value={form.fullName} onChangeText={(t) => setForm({ ...form, fullName: t })} />
        <TextInput style={styles.input} placeholder="Personal Email *" keyboardType="email-address" autoCapitalize="none" value={form.email} onChangeText={(t) => setForm({ ...form, email: t })} />
        ...
        <TextInput style={styles.input} placeholder="Graduation Year (e.g., 2022)" keyboardType="numeric" value={form.graduationYear} onChangeText={(t) => setForm({ ...form, graduationYear: t })} />
        <TextInput style={styles.input} placeholder="Current Company *" value={form.company} onChangeText={(t) => setForm({ ...form, company: t })} />
        <TextInput style={styles.input} placeholder="Job Role (e.g., SDE II)" value={form.jobRole} onChangeText={(t) => setForm({ ...form, jobRole: t })} />

        <PrimaryButton title="Create Alumni Account" onPress={handleRegister} isLoading={loading} style={{ marginTop: 20 }} />
      </ScrollView>
